...
**Phase 1 is complete, tested, and shipped** (tagged **v1.1.1**; a GitHub Action builds & pushes the image to **ghcr.io** on every `v*` tag push), and **Phase 2.1 (Tool Calling Runtime) is implemented, mocked-tested, and released** (v1.2.0 / v1.2.1). Phase 1 delivers:
...
`tests/conftest.py` provides an in-memory `repo` fixture and `FakeLLM` / `RecordingLLM` fakes. **All 106 tests pass, all mocked** — nothing ever talks to the real LLM endpoint or Telegram (mock the SDK's `create` and the PTB handlers). Coverage:
