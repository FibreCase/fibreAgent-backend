...
- `AgentService.process_message()` now accepts `str | AgentMessage`; `ChatMessage.content` (in *both* `agent/context.py` and `llm/client.py`) is widened to `str | list[dict]`. At this point an image rides only in the **current** turn (phase 2.2 — below — is what persists it).
...
Phase 2.2 (Persistent Image Attachment Storage) makes those Telegram photos **survive restarts**: instead of living only in the in-memory request, each image is written to a content-addressed blob store and its metadata to a new `attachments` table, and **history** images inside the `MAX_CONTEXT_MESSAGES` window are re-attached into the OpenAI context on later turns. On top of the same layers, without touching the tool runtime:
...
One attachment-storage knob (phase 2.2):
...
- **`agent/context.py`** — `build_context(system, history, max_n)` = system prompt + most recent N **messages** (N is a *message* count, not tokens; `MAX_CONTEXT_MESSAGES`). `ChatMessage.content` is **`str | list[dict]`**: a plain `str` for every text-only message, or a `list` of OpenAI content parts for a message that carries an image — the *current* multimodal turn (built by `llm/message_converter.py`) **or a history turn whose image attachments were rehydrated** (phase 2.2). It also carries optional `tool_calls` / `tool_call_id` (both `None` for every Phase-1 message, so `to_dict()` is unchanged there). This is the single place to swap in token-based context management later.
...
- **`database/`** — `models.py` (ORM), `session.py` (engine/session factory + `init_db`), `repository.py` (the only layer that touches ORM; handlers never write SQL). One Telegram chat = one conversation, keyed by `telegram_chat_id`. `Message.role` already allows `tool` (ahead of phase 2), but tool turns are never written. Phase 2.2 adds the `Attachment` table (see above) and a `Message.attachments` relationship; the repository gains `add_message_attachments` (one commit per message), `get_messages_with_attachments` (eager-loads attachments via `selectinload` and returns **detached** `MessageWithAttachments`/`AttachmentRef` records — safe after session close, no lazy loading), and `attachment_sha256_for_chat` / `distinct_attachment_sha256` (the digest sets `reset()` uses to reclaim blobs). `get_messages()` is unchanged and remains the plain-text path. `reset_conversation` now explicitly deletes the old conversation's attachments and messages. `init_db` (`create_all`) creates the new table on a fresh DB *and* adds it to an existing v1.3.0 DB that only has `conversations`/`messages` — no data loss, no manual `data/agent.db` wipe, no Alembic.
...
- **A Telegram photo's text lives in `message.caption`, not `message.text`** (which is `None` for a bare photo). The handler filter is `TEXT | PHOTO`, and `telegram/media.py::normalize_message` reads the caption into a `TextContent`. An image-only message therefore persists an empty-string user turn — that is correct, not a bug. (Phase 2.2: the *image itself* is persisted as a blob + `attachments` row; the empty text is just the missing caption.)
...
- **Rehydration is window-scoped**: the service truncates history to the recent window *before* rehydrating, so an out-of-window image blob is **never read from disk** and never sent. Don't "fix" this by rehydrating everything and truncating after — it would read (and could ship) images the user can no longer see.
...
`tests/conftest.py` provides an in-memory `repo` fixture and `FakeLLM` / `RecordingLLM` fakes. **All 145 tests pass, all mocked** — nothing ever talks to the real LLM endpoint or Telegram (mock the SDK's `create`, the PTB handlers, `PhotoSize.get_file`, and the LLM for attachment tests). Coverage:
...
- **Image persistence** moved to **phase 2.2**: images are now written to the content-addressed attachment store and re-attached to history (see *Attachment storage limitations* below). The phase-2.1.x "images are in-memory only / not persisted" behaviour still applies *only* when `attachment_store` is `None`.
...
## Attachment storage limitations (phase 2.2)
...
- **In-window only.** A history image reaches the LLM only if its message is inside `MAX_CONTEXT_MESSAGES`. Beyond that it is simply not rehydrated (not an error).
...
- **No token accounting.** `MAX_CONTEXT_MESSAGES` counts *messages*, not tokens; a window of images can be a large payload. Token-budgeted trimming of attachments is explicitly deferred (see Extending).

## Extending (phase 2.3+, not yet)

The tool runtime, multimodal input, and persistent image attachment storage are
now built and stable; the next additions should slot in as **Tool Providers**
...
foundation *and* the phase-2.2 attachment store — add a new `ContentPart`
...
- **Context Management** (recommended next step): today the context window is
  truncated by *message count* (`MAX_CONTEXT_MESSAGES`), so an image rides in or
  out of the window with its message as a unit. Add token-budgeted trimming with
  an attachment cost/count strategy (e.g. keep the most recent N images in
  window, downgrade older ones to a text reference) so long, image-heavy
  conversations stay within a token budget.
