...
| 启动报 `ConfigError`（工具/预算/超时相关） | 某个安全/预算配置不合法：`TOOL_PERMISSION_OVERRIDES` 条目缺 `=`、空工具名、空/非法权限值、重复工具名、工具名含 `[A-Za-z0-9_-]` 之外字符，或某个预算/超时为 0/负数/非整数。修好再启动——**坏掉的安全配置不会被静默忽略**。 |
...
| 工具调用一直没结果 / 卡在审批 | `ask` 工具在等你的 `Approve`/`Deny`；超过 `TOOL_APPROVAL_TIMEOUT_SECONDS` 会按「审批已过期」处理（不执行）。工具执行超过 `TOOL_TIMEOUT_SECONDS` 会按「工具超时」处理。用 `/tool_audit` 查最近一次工具的 `event_type`/`code`。 |
