...
**MCP 已按此模式接入**（`mcp/` 包，Streamable HTTP + stdio）：启动时发现远程端点或后端 spawn 的本地 stdio 子进程的工具并包成标准 `Tool`（`mcp_<server>__<remote>`、默认 `ask`），注册进同一个 registry，从而完全复用执行边界——见 [远程 MCP 工具](mcp.md)。未来的 **SSH / Docker / Pi** 仍是同一个模式：各是一个 `Tool`（或一个产出若干工具的小 provider），subprocess/网络都封装在工具**内部**、绝不进 loop，并在有副作用时走审批。
