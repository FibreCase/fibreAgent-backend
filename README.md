...
已在 Telegram 上跑通的最小个人 Agent backend。**能做的**：工具调用 + 工具安全、图片输入/持久化、上下文预算管理、显式长期记忆、Markdown 渲染。**有意不做**：仅 3 个只读工具（无 shell/文件/联网/SSH）、图片仅本地磁盘仅照片、估算是模型无关的保守值非计费 token、记忆是显式 + 纯词法（跨语言召回弱）。

**下一步（建议顺序）**：新的 `ContentPart` 类型（File / Audio / Video / Sticker，复用同一套附件存储）；新的工具能力（如 MCP）以 Tool Provider 接入同一接口，有副作用的默认进 `ask` 审批。
